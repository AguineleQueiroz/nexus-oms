<template>
  <div class="order-detail-view">
    <div class="detail-header">
      ...
      <h1 v-if="order" class="page-title">{{ order.customer_name }}</h1>
...
async function pay() {
  order.value = await api.payOrder(route.params.id as string)
}

async function advance() {
  order.value = await api.advanceOrder(route.params.id as string)
}

async function cancel() {
  order.value = await api.cancelOrder(route.params.id as string)
}
